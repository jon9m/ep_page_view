...
exports.aceDomLineProcessLineAttributes = function (name, context) {
  if (context.cls.indexOf("pageBreak") !== -1) {
    var type = "pageBreak";
  }
  var tagIndex = context.cls.indexOf(type);
  if (tagIndex !== undefined && type) {
    top.console.log("Incrementing page number tagIndex " + tagIndex);
    // NOTE THE INLINE CSS IS REQUIRED FOR IT TO WORK WITH PRINTING!   Or is it?
    var modifier = {
      preHtml: '<div class="pageBreak" contentEditable=false style="page-break-after:always;page-break-inside:avoid;-webkit-region-break-inside: avoid;position:relative;"><div class="pageNumberDiv" style="position:absolute;color:red;bottom:75px;right:100px;"></div>',
      postHtml: '</div>',
...
exports.aceEditEvent = function (hook, callstack, editorInfo, rep, documentAttributeManager) {
  ...
  // if(callstack.callstack.type == "handleClick" || callstack.callstack.docTextChanged){
  checkPageNumber();
  // }
...
reDrawPageBreaks = function () {
  ...
  $(HTMLLines).each(function () { // For each line
    ...
    if (computedBreak) {
      ...
      if (!isAlreadyPageBreak) {
        ...
        $(this).append("<div class='pageBreakComputed' contentEditable=false style='position:relative;'><br></div>");
  ...
  //  checkPageNumber(HTMLLines, lines, lineNumber);
...
checkPageNumber = function () {
  ...
  var pageNumberX = $('iframe[name="ace_outer"]').width() * 0.6;
